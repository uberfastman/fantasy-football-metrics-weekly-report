...
class PointsByPosition(object):
    def __init__(self, league: BaseLeague, week_for_report: int):
        ...
        self.roster_slot_counts: Dict[str, int] = {
            k: v for k, v in league.roster_position_counts.items() if v != 0
        }
        ...
        for team in season_average_points_by_position_dict:
            ...
            for week in points_by_position:
                for position in week:
                    ...
                    if position_points:
                        season_average_points_by_position[position[0]] = (
            ...
            for position in list(season_average_points_by_position.keys()):
                season_average_points_by_position_list.append(
                    [
                        position,
                        season_average_points_by_position.get(position)
                        / len(points_by_position),
                    ]
                )
            season_average_points_by_position_list = sorted(
                season_average_points_by_position_list, key=lambda x: x[0]
            )
            season_average_points_by_position_dict[team] = (
                season_average_points_by_position_list
            )

        return season_average_points_by_position_dict

    def _get_points_for_position(
        self, players: List[BasePlayer], position: str
    ) -> float:
        ...
        for player in players:
            if (
                ...
                or (
                    ...
                    in self.flex_positions_dict.get(position, [])
                )
    ...
    def _execute_points_by_position(
        self, team_name: str, roster: List[BasePlayer]
    ) -> List[List[Any]]:
        logger.debug(f'Calculating points by position for team "{team_name}".')

        player_points_by_position = []
        starting_players = [
            p for p in roster if p.selected_position not in self.bench_positions
        ]
        for slot in list(self.roster_slot_counts.keys()):
            if slot not in self.bench_positions and slot not in self.flex_types:
                player_points_by_position.append(
                    [slot, self._get_points_for_position(starting_players, slot)]
                )

        player_points_by_position = sorted(
            player_points_by_position, key=lambda x: x[0]
        )
        return player_points_by_position

    def get_weekly_points_by_position(
        self, teams_results: Dict[str, BaseTeam]
    ) -> List[List[Any]]:
        ...
        for team_result in teams_results.values():
            for slot in list(team_roster_slot_counts.keys()):
                if slot in self.flex_types:
                    if not set(self.flex_positions_dict.get(slot)).intersection(
                        set(team_roster_slots)
                    ):
            ...
            player_points_by_position = self._execute_points_by_position(
                team_result.name, team_result.roster
            )
            weekly_points_by_position_data.append(
                [team_result.team_id, player_points_by_position]
            )
