...
class CoachingEfficiency(object):
    def __init__(self, league):
        ...
        self.flex_positions_dict: Dict[str, List[str]] = (
            self.league.get_flex_positions_dict()
        )
        self.roster_primary_slots: Set[str] = set(self.roster_active_slots).difference(
            self.flex_positions_dict.keys()
        )
        self.roster_flex_slots: Set[str] = set(self.roster_active_slots).intersection(
            self.flex_positions_dict.keys()
        )
    ...
    def _is_player_ineligible(self, player: BasePlayer, week, inactives):
        if player.points != 0.0:
            ...
            return (
                ...
                or generate_normalized_player_key(
                    player.full_name, player.nfl_team_abbr
                )
                in inactives
            )

    def _get_player_open_positions(
        self, player: BasePlayer, optimal_lineup: Dict[str, RosterSlot]
    ):
        eligible_positions = list(
            player.eligible_positions.intersection(set(self.roster_active_slots))
        )
        ...
        open_positions = [
            pos
            for pos, roster_slot in optimal_lineup_open_positions.items()
            if not roster_slot.is_full()
        ]
    ...
    def _create_open_slot_if_possible(
        ...
        players_with_open_slots: Dict[
            str, List[Dict[str, Union[BasePlayer, List[str]]]]
        ],
        ...
        for pos, assigned_players_info in players_with_open_slots.items():
            for assigned_player in assigned_players_info:
                ...
                if len(open_positions) > 0:
                    player: BasePlayer
                    for player_ndx, player in enumerate(
                        optimal_lineup.get(pos).assigned_players
                    ):
                        # check if any players in position match the potentially replaceable player
                        if (
                            assigned_player["assigned_player"].player_id
                            == player.player_id
                        ):
                            # remove the replaceable player from their current optimal lineup position
                            movable_player = optimal_lineup.get(pos).remove_player(
                                player_ndx
                            )
                            ...
                            optimal_lineup.get(open_positions[0]).add_player(
                                movable_player
                            )
    ...
    def _assign_player_to_optimal_slot(
        self, player: BasePlayer, optimal_lineup: Dict[str, RosterSlot]
    ):
        ...
        eligible_primary_positions = list(
            player.eligible_positions.intersection(eligible_positions).intersection(
                self.roster_primary_slots
            )
        )
        eligible_flex_positions = list(
            player.eligible_positions.intersection(eligible_positions).intersection(
                self.roster_flex_slots
            )
        )

        assigned_pos = None
        point_diffs: Dict[str, float] = {}
        players_with_eligible_open_slots: Dict[
            str, List[Dict[str, Union[BasePlayer, List[str]]]]
        ] = defaultdict(list)
        for eligible_pos in eligible_primary_positions + eligible_flex_positions:
            if not optimal_lineup.get(eligible_pos).is_full():
                ...
                point_diffs[eligible_pos] = (
                    player.points
                    - optimal_lineup.get(eligible_pos).assigned_players[-1].points
                )
                for assigned_player in optimal_lineup.get(
                    eligible_pos
                ).assigned_players:
                    players_with_eligible_open_slots[eligible_pos].append(
                        {
                            "assigned_player": assigned_player,
                            "open_positions": self._get_player_open_positions(
                                assigned_player, optimal_lineup
                            ),
                        }
                    )
        ...
        if assigned_pos:
            ...
            if max_point_diff <= 0:
                ...
                return self._create_open_slot_if_possible(
                    player, players_with_eligible_open_slots, optimal_lineup
                )

            # collect all positions that share the max point difference with the player
            max_point_diff_positions = [
                pos
                for pos, point_diff in point_diffs.items()
            ...
            if len(max_point_diff_positions) == 1:
                ...
                max_point_diff_primary_positions = list(
                    set(max_point_diff_positions).intersection(
                        eligible_primary_positions
                    )
                )
            ...
            replaced_player = optimal_lineup.get(
                max_point_diff_position
            ).remove_player()
    ...
    def execute_coaching_efficiency(
        ...
        logger.debug(
            f'Calculating week {week} coaching efficiency for team "{team_name}".'
        )
        ...
        optimal_score = round(
            sum(
                [
                    p.points
                    for roster_slot in optimal_lineup.values()
                    for p in roster_slot.assigned_players
                ]
            ),
            2,
        )
        ...
        if dq_eligible:
            ...
            ineligible_efficiency_player_count = len(
                [
                    ...
                    if self._is_player_ineligible(p, week, inactive_players)
                ]
            )
            ...
            if Counter(self.roster_active_slots) == Counter(positions_filled_active):
                ...
                if ineligible_efficiency_player_count < math.ceil(
                    ...
                    self.coaching_efficiency_dqs[team_name] = (
                        ineligible_efficiency_player_count
                    )
            else:
                efficiency_disqualification = True
