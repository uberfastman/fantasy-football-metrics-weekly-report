...
class BeefFeature(BaseFeature):
    def __init__(
        ...
        super().__init__(
            "beef", "https://api.sleeper.app/v1/players/nfl", week_for_report, data_dir, refresh, save_data, offline
        )
    ...
    def _get_feature_data(self):
        ...
        for player_sleeper_key, player_data_json in nfl_player_data.items():
            ...
            # excludes defences with "DEF" as beef data for defences is generated by rolling up all players on that
            # defense
            if (
                player_data_json
                ...
                if (
                    position_types
                    and not positions.intersection(("OL", "RB", "WR", "TE"))
                    ...
                    if player_beef_dict.get("team") not in self.feature_data.keys():
                        ...
                        weight = self.feature_data[player_beef_dict.get("team")].get("weight") + player_beef_dict.get(
                            "weight"
                        )
    ...
    def _get_player_beef_stats(
        self, player_first_name: str, player_last_name: str, player_team_abbr: str, key_str: str
    ) -> float:
        ...
        if player_first_name and player_last_name:
            ...
            if any(punc in player_first_name for punc in self.first_name_punctuation) or any(
                suffix in player_last_name for suffix in self.last_name_suffixes
            ):
