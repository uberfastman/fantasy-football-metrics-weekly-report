...
from utils.report_tools import user_week_input_validation

logger = logging.getLogger(__name__)

# Suppress CBS API debug logging
...
class LeagueData(object):
    ...
    def __init__(self,
        ...
        if not self.dev_offline:
            ...
            cbs_auth_dir = os.path.join(base_dir, self.config.get("CBS", "cbs_auth_dir"))
            cbs_access_token_file = os.path.join(cbs_auth_dir, "token.json")

            if not os.path.exists(cbs_access_token_file):
                ...
                login_url = "https://api.cbssports.com/general/oauth/mobile/login/"
                headers = {
                ...
                body = urllib.parse.urlencode({
                    "user_id": cbs_auth_json.get("user_id"),
                ...
                self.cbs_access_token = response_json.get("body").get("access_token")

                with open(cbs_access_token_file, "w") as token:
                    token.write(json.dumps(
                        {
                            "access_token": self.cbs_access_token
                        },
                        indent=2
                    ))
            else:
                with open(cbs_access_token_file) as token:
                    self.cbs_access_token = json.load(token).get("access_token")

        user_teams = self.query(
            "mobile/users/teams?version=3.0",
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-user-leagues.json",
            "fantasy_teams",
            needs_league_id=False
        )

        for user_team in user_teams:
            if user_team.get("league_id") == self.league_id:
                self.league_url = user_team.get("base_url")

        # league details/info
        league_details = self.query(
            "league/details?version=3.0",
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-league-details.json",
            "league_details"
        )

        self.current_week = int(league_details.get("current_period"))

        # TODO: FIND CBS LEAGUE WITH PLAYOFFS TO TEST ON
        self.num_playoff_slots = 0
        self.num_regular_season_weeks = league_details.get("scoring_periods")
        ...
        league_rules = self.query(
            "league/rules?version=3.0",
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-league-rules.json",
            "rules"
        )

        self.roster_positions = league_rules.get("roster").get("positions")

        league_playoff_settings = self.query(
            "league/playoff-settings?version=3.0",
            os.path.join(self.data_dir, str(self.season), str(self.league_id)),
            str(self.league_id) + "-playoff-settings.json",
            "playoff_settings"
        )
    ...
    def query(self, url, file_dir, filename, content_key=None, needs_league_id=True):

        file_path = os.path.join(file_dir, filename)

        if not self.dev_offline:

            request_url = "https://api.cbssports.com/fantasy/{}&sport=football&response_format=json{}".format(
                url,
                "&league_id={}".format(self.league_id) if needs_league_id else ""
            )

            # # UNCOMMENT IF USING RUBY SCRIPT FOR CBS AUTHENTICATION!
            # response = requests.get(
            #     "https://api.cbssports.com/fantasy/" + url + "&response_format=json&access_token={}".format(
            #         self.cbs_access_token))

            auth_header = {
                "Authorization": self.cbs_access_token
            }
        ...
        return response_json.get("body").get(content_key) if content_key else response_json.get("body")
...
if __name__ == '__main__':
    ...
    local_data_dir = os.path.join(local_base_dir, local_config.get("Configuration", "data_dir"))

    cbs_league = LeagueData(
        3,
        "nfcafcdivide",
        2020,
        local_config,
    ...
    # # NFL pro teams
    # query_response = query(
    #     "pro-teams?version=3.0"
    # )

    # # league managers
    # query_response = query(
    #     "league/owners?version=3.0"
    # )
